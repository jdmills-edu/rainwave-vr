		...
		function maximumDancing() {
			...
			tween.start();
			for ($i=0; $i < volumeObjects.length; $i++){
				volumeObjects[$i].position.z = bars[0].position.z + 100;
			}
			volumeDrag.addBehavior(altspace.utilities.behaviors.Drag({ x: {min: -100, max: 100}, z: {min: volumeSlider.position.z, max: volumeSlider.position.z}, y: false}));
			
			partyPedastal.position.z = bars[0].position.z + 100;
			partyButton.position.z = partyPedastal.position.z;
			
			helpCylinder.position.z = bars[0].position.z + 120;
			helpCylinder.rotation.x = (30) * (Math.PI/180);
			helpCylinder.position.y = carpet.position.y + 15;
			helpButton.position.z = helpCylinder.position.z + 10;
			helpButton.rotation.x = (-60) * (Math.PI/180);
			helpButton.position.y = carpet.position.y + 10;
		}
		function disablePartyMode(){
			...
			//reset control objects
			for ($i=0; $i < volumeObjects.length; $i++){
				volumeObjects[$i].position.z = controlPanel.position.z;
			}
			
			volumeDrag.addBehavior(altspace.utilities.behaviors.Drag({ x: {min: -100, max: 100}, z: {min: volumeSlider.position.z, max: volumeSlider.position.z}, y: false}));
			
			partyPedastal.position.z = 470;
			partyButton.position.z = partyPedastal.position.z;
			
			helpCylinder.position.z = controlPanel.position.z + 160;
			helpCylinder.rotation.x = (10) * (Math.PI/180);
			helpCylinder.position.y = carpet.position.y + 10;
			helpButton.position.z = controlPanel.position.z + 170;
			helpButton.rotation.x = (-80) * (Math.PI/180);
			helpButton.position.y = carpet.position.y + 5;
		...
		//volume
		volumeDrag.addEventListener('cursordown', function(){
			stream.volume = (volumeDrag.position.x / 200) + .5;
		});
		volumeDrag.addEventListener('cursorup', function(){
			stream.volume = (volumeDrag.position.x / 200) + .5;
		});
